...
public class ResourceManager : MonoBehaviour
{
    ...
    {
        BaseDownloadingURL = GetRelativePath();

        ioo.gameManager.message += BaseDownloadingURL + "\n";

        // Initialize AssetBundleManifest which loads the AssetBundleManifest object.
        var request = Initialize("AssetBundles");
        if (request != null)
    ...
    /// 载入UI素材
    /// </summary>
    public void LoadUIAsset(string abname, string assetname, LuaFunction func)
    {
        StartCoroutine(OnLoadUIAsset(abname, assetname, func));
    }
    ...
    /// 载入素材
    /// </summary>
    public void LoadAsset(string abname, string assetname, Action<GameObject> callback = null)
    {
        StartCoroutine(OnLoadAsset(abname, assetname, callback));
    }
    ...
    /// 载入场景
    /// </summary>
    public void LoadLevel(string abname, string levelName, bool isAdditive, Action<GameObject> callback = null)
    {
        StartCoroutine(OnLoadLevel(abname, levelName, isAdditive, callback));
    }
    ...
    {
        // 备注：streamingAssetsPath 只可读  persistentDataPath 可读可写
        if (Application.isMobilePlatform)
            //return Application.streamingAssetsPath + "/" + Const.AssetDirName + "/";
            return "file://" + Util.DataPath;
        else if (Application.isWebPlayer)
            return System.IO.Path.GetDirectoryName(Application.absoluteURL).Replace("\\", "/") + "/StreamingAssets/" + Const.AssetDirName + "/";
        else // For standalone player.
            return "file://" + Application.streamingAssetsPath + "/" + Const.AssetDirName + "/";
    }
} 
